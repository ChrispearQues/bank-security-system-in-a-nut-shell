...
# One row = one issued token, i.e. one active login (一张已发出的通行证).
    ...
    user_id = Column(int, ForeignKey("users.id"), nullable = False, index = True) # Whose token this is.
    tocken = Column(String, unique = True, index = True, nullable = False) # The pass itself, generated with secrets.token_urlsafe(32) at login time.
